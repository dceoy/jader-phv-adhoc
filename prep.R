...
if(! file.exists('input/csv/dt_soc.csv')) {
  sql_ade = 'SELECT DISTINCT
               drug, soc_code, case_id, year, age, sex
             FROM
               ade;'
  sql_soc = 'SELECT DISTINCT
               s.soc_code AS soc_code, soc_name, soc_kanji
             FROM
               soc s
             INNER JOIN
               soc_j sj ON sj.soc_code == s.soc_code;'
  sql_ct22 = 'SELECT
                t123.drug AS drug, t123.soc_code AS soc_code, a, a_b - a AS b, a_c - a AS c, t - a_b - a_c + a AS d
              FROM
                (
                  (
                    (
